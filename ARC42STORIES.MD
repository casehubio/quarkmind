...
| Adaptive plugin selection | `casehub-engine` binding conditions | L5 | 🔲 pending (#157) |
| Trust routing | `casehub-ledger` (EigenTrust, lightweight) | L6 | 🔲 pending (#158) |
...
Known scope: plugin outcome recording (plugin ID, decision, game state context, result); EigenTrust reputation scoring; async non-blocking writes; in-memory backend for mock/emulated profiles.
...
🔲 Full Chapter entry at C4 close. Blocked on #157.
...
🔲 Full entry at C3 close. Known scope: plugin outcome recording without cryptographic signing; EigenTrust reputation scoring; async non-blocking writes at game-loop granularity; in-memory backend for mock/emulated profiles. Lightweight mode required — tamper-evident guarantees not needed for game agent decisions.
...
**Completed:** 🔲 pending (#157)

🔲 Full entry at C4 close. Known scope: `casehub-engine` binding conditions evaluate game state per tick; `TacticsTask` dispatches only when enemy units present; `ScoutingTask` dispatches only when scouting target is incomplete; eliminates hardwired all-plugins-every-tick dispatch.
...
🔲 Full entry at C5 close. Known scope: `TrustWeightedAgentStrategy` selects among competing plugin implementations by EigenTrust score; trust scores accumulate from plugin outcome attestations written in L4; routing quality improves automatically across game sessions.
