...
**Prefix:** QM
**Status:** L1 conceptual + L2, L3, L4, L5, L6, L7 complete.
...
| Issue | `#NNN` or `casehubio/quarkmind#NNN` | `#155` | GitHub Issues |
...
| `ocraft-s2client-protocol` | 🔲 Not verified — protobuf-java + jackson only; no Vert.x/RxJava2 (#185 removed transport layer). Tracked in `NATIVE.md`. |
| `scelight-mpq` + `scelight-s2protocol` | ❌ JVM-only — tracked in `NATIVE.md` |
| `casehub-blocks` | ✅ Pure Java — provides generic summarisation framework types (`io.casehub.blocks.summarisation`): `EventLevel`, `LevelEvent`, `WindowPolicy`, `EventAccumulator`, `EventStreamBus`, `Summariser`, `SummarisationRunner`. Extracted via casehubio/blocks#27. |
...
| casehub-qhorus | `casehub-qhorus` | L3 | ✅ complete (#155) |
| casehub-ledger | `casehub-ledger` (lightweight mode — casehubio/ledger#114) | L4 | ✅ complete (#156) |
| Adaptive plugin selection | `casehub-engine` binding conditions | L5 | ✅ complete (#157) |
| Trust routing | `casehub-ledger` + QuarkMind-native `StrategyTrustRouter` (Bayesian Beta CAPABILITY per opponent context; `casehub-engine-ledger` not used — no `casehub-engine` integration; EigenTrust inert, ADR-0009) | L6 | ✅ complete (#158) |
| Comparison baseline | *(none — analysis only)* | L7 | ✅ complete (#159) |
...
- C3 before C4: layer sequence — L4 (ledger/outcome tracking) precedes L5 (adaptive plugin selection) in delivery order; no runtime dependency between `PluginOutcomeAuditor` and the requires()/activateIf() dispatch gates
- C4 before C5: C5 extends C4's `activateIf()` gate mechanism — `DroolsStrategyTask` gains `strategySelector.isSelected()` to enforce exactly-one-strategy-per-tick across competing implementations; the gate framework must exist before trust routing can add to it
...
### QuarkVille Module Structure (quarkmind-ville)

First non-SC2 consumer of quarkmind-core agency SPIs. Client/server architecture — central Quarkus game server owns world state, AI agent clients connect via WebSocket.

```
quarkmind-ville/                          ← parent POM (aggregator)
├── quarkmind-ville-protocol/             ← shared message types
│   io.quarkmind.ville.protocol/
│     VilleIntent (sealed: Move, Talk, Rest, Emote) implements Intent
│     VillePerception implements WorldPerception
│     CharacterSnapshot, Position, VilleEvent
│     VilleClientMessage (sealed: Connect, IntentMessage, Thought)
│     VilleServerMessage (sealed: Perception, ObserverPerception, Result, ThoughtBroadcast)
├── quarkmind-ville-server/               ← game server (Quarkus)
│   io.quarkmind.ville.server/
│     WorldState, CharacterState, GameTick, VilleNeedDefinition
│     VilleSocket (@WebSocket /ws/ville), VilleServer (@ApplicationScoped)
│     PerceptionBuilder (range-filtered agent perception, observer perception)
├── quarkmind-ville-agent/                ← agent client (standalone)
│   io.quarkmind.ville.agent/
│     VilleWorldBridge implements WorldBridge<VillePerception, VilleIntent>
│     VilleAgencyLoop implements AgencyLoop (LLM-driven, no CaseEngine)
│     AgentRunner (main entry, virtual-thread multi-agent)
└── godot/                                ← Godot 4 visual client (observer)
```

quarkmind-core SPIs exercised: `WorldBridge`, `AgencyLoop`, `NeedState`, `NeedDefinition`, `DispositionNeedModifier`, `Intent`, `IntentQueue`, `AgencyContext`, `WorldPerception`.
...
### Scenario — Coach mode tick (game.mode = "coach")

In coach mode, `GameTickExecutor` gates off action plugins (strategy, tactics, economics, strategy-routing) via `activateIf()` predicates checking `game.mode`. Observation plugins (scouting, moment detection, summarisation) and commentary run in both modes. The coaching pipeline adds two per-tick operations: `CoachingComplianceEvaluator.evaluate()` checks open commitments against game state deltas, and `CoachingTriggerBuilder.build()` fires COMMAND triggers on coaching moments. Advisory triggers and milestone evaluation are suppressed.
...
Native image target:
  GraalVM 25; non-native deps (scelight) tracked in NATIVE.md
  ocraft-s2client-bot/api removed (#185); ocraft-s2client-protocol (protobuf + jackson) needs native verification
  Build: mvn package -Dnative (scelight blocks native; ocraft transport blocker eliminated by #185)
...
| 2 | Plugin Intel Channels | Game Agent Coordination | + L3 | Low | ✅ complete (#155, #177) |
| 3 | Outcome tracking | Game Agent Coordination | + L4 | Medium | ✅ complete (#156) |
| 4 | Adaptive plugin selection | Game Agent Coordination | + L5 | Medium | ✅ complete (#157) |
| 5 | Trust-weighted routing | Game Agent Coordination | + L6 | Medium | ✅ complete (#158) |
...
| L2 casehub-engine | High | Low | Low | Medium | Low | Low |
| L3 casehub-qhorus | — | Low | — | — | — | — |
| L4 casehub-ledger | — | — | Medium | Low | Medium | — |
| L5 Adaptive selection | — | — | — | Medium | Low | — |
| L6 Trust routing | — | — | — | — | High | — |
| L7 Comparison | — | — | — | — | — | Low |

L2 participates in every Chapter — it is the foundational dispatch layer. L3 and L5 are each additive (one Chapter at Low/Medium, stable thereafter). L4 participates in three Chapters — it provides the audit record (C3) and the trust data that C5 reads; C4's conditional dispatch gates depend on CaseFile key presence and broker state, not L4 attestations.
...
- C3 before C4: layer sequence — L4 (ledger) precedes L5 (adaptive plugin selection) in the delivery sequence; no runtime dependency exists between outcome tracking and the conditional dispatch gates
- C4 before C5: C5 extends C4's `activateIf()` gate mechanism — `DroolsStrategyTask` gains `strategySelector.isSelected()` in C5, enforcing exactly-one-strategy-per-tick when three implementations compete; the gate framework must exist before trust routing can add to it
...
**What this delivers**
`ScoutingIntelBroker` dual-stack delivers typed scouting intel via two independent paths: a synchronous in-memory store (Stack 1) for plugin-to-plugin delivery, and a Qhorus channel (Stack 2, `quarkmind-scouting-intel`) for external advisory consumers (LLM advisors, Commentator). Plugins — `DroolsTacticsTask`, `DroolsStrategyTask`, `FlowEconomicsTask` — implement `ScoutingIntelConsumer` and read via `broker.current()` on Stack 1. `TacticsIntelCache` and `TacticsMessageBridge` removed; `MessageObserver` wiring eliminated.
...
**What this delivers**
Every plugin decision is recorded via casehub-ledger: plugins fire `PluginDecisionEvent` when their decision state changes, and `PluginOutcomeAuditor` observes asynchronously (`@ObservesAsync`) to write a ledger entry via `outcomeRecorder.record()`. Bayesian Beta trust scores accumulate per plugin over time. Writes are async non-blocking — game-loop latency is unaffected. In-memory backend used for mock and emulated profiles (lightweight mode — tamper-evident guarantees not needed for game agent decisions). EigenTrust inert; single-attestor mode is correct for this domain (ADR-0009).
...
| L2 casehub-engine | Low — dispatch unchanged; L2 is the substrate whose plugin execution triggers `PluginDecisionEvent`, enabling async ledger recording |
...
- C2–C5 §9.3 chapter entries: complete (#198)
...
#### Phase 1 engine migration (#193)

All four seam interfaces (`StrategyTask`, `TacticsTask`, `EconomicsTask`, `ScoutingTask`) now extend both `io.casehub.core.TaskDefinition` (poc, retained for Phase 1 bridge compatibility) and `io.quarkmind.agent.TaskDefinition` (QuarkMind's own interface). Every plugin implementation adds `execute(CaseContext)`, `activateIf()`, `requires()`, and `produces()` — the canonical API for Phase 2. Bridge methods (`execute(CaseFile)`, `canActivate(CaseFile)`, `entryCriteria()`) delegate to the new implementations so the existing poc `CaseEngine.createAndSolve()` path continues to work unchanged until Phase 2. `CaseFileContext` adapter wraps a poc `CaseFile` as `CaseContext` for bridge use (Phase 1 shim, deleted in Phase 2). This migration is purely additive: no plugin behaviour changes, no poc dependency removal.
...
**Completed:** ✅ complete (#155)

Dual-stack intel delivery: `ScoutingIntelBroker` serves as both typed in-memory store (Stack 1 — synchronous, for plugin consumers) and Qhorus channel coordinator (Stack 2 — async, for LLM advisory consumers). `DroolsTacticsTask` no longer implements `MessageObserver` — reads threat position from broker via constructor-injected `ScoutingIntelBroker`. `TacticsIntelCache` removed. New consumers: `DroolsStrategyTask` and `FlowEconomicsTask` implement `ScoutingIntelConsumer` and read from broker. `ScoutingIntelBroker.refreshAll()` hot-reloads subscription union (#178). Qhorus channel (`quarkmind-scouting-intel`) is the advisory surface for LLM advisors (#180) and Commentator/Coach (#181). `TacticsMessageBridge` deleted. Full Chapter entry at C2 close.
...
**Ordering dependency** — `ENEMY_ARMY_SIZE` on `StrategyTask`: scouting always writes this key (even as 0), so strategy always runs after scouting. The criterion enforces execution order without causing genuine skipping.

**Note on code/ARC alignment:** After #169, the code diverged from this documented graph by adding `ENEMY_POSTURE` and `TIMING_ATTACK_INCOMING` to `DroolsStrategyTask.entryCriteria()`. The dual-stack redesign (#177) restored alignment: strategy's `entryCriteria()` is `{READY, ENEMY_ARMY_SIZE}` and posture/timing are now read from the `ScoutingIntelBroker`. The code again matches this documented dependency graph.

**Conditional gate** — `broker.current(THREAT_POSITION).isPresent()` on `TacticsTask`: `DroolsScoutingTask` calls `broker.update(ThreatPosition)` only when enemies are visible. When no enemies are visible, the broker is empty and `DroolsTacticsTask.canActivate()` returns false. This replaced the former `NEAREST_THREAT` CaseFile key gate (#177, #179). `NEAREST_THREAT` constant and CaseFile write removed; intel flows via `ScoutingIntelBroker` only.

**Dependency graph (updated #177, #179):**
```
...
**Completed:** ✅ 2026-06-10 (#158)

Three competing `StrategyTask` implementations now exist. At game start, `StrategyTrustRouter.select()` runs against all CDI-discovered strategy candidates for the `STRATEGY_VS_UNKNOWN` context. At the first mid-game checkpoint (when `DroolsScoutingTask` publishes a non-UNKNOWN `ENEMY_POSTURE` via `ScoutingIntelBroker`), the router re-selects for the classified opponent context (`vs.aggressive`, `vs.economic`, `vs.defensive`). `StrategySelector` holds the selection; each strategy's `canActivate()` gates on it — enforcing that exactly one fires per tick. (#300: `EnemyPostureClassifiedEvent` removed — posture now flows through `ScoutingIntelBroker` and the `TacticalPosture` cascade.)

`StrategyTrustRouter` implements the four-phase trust maturity model using `casehub-ledger`'s `TrustGateService` (not `casehub-engine-ledger` — QuarkMind uses `casehub-core`, not `casehub-engine`). `BOOTSTRAP` phase (phaseScore=0.5, below minimum QUALIFIED phaseScore of 0.838) ensures any qualified candidate beats any unproven one. The designated fallback (`strategy.drools`) wins all ties and is exempt from BORDERLINE exclusion.

`GameOutcomeRecorder` observes `GameStopped` synchronously — records the completed game's strategy and opponent context via `OutcomeRecorder.record()`; trust scores materialise via `IncrementalTrustUpdateObserver` at `TransactionPhase.AFTER_SUCCESS`. `LedgerLifecycleAdapter` was removed — it was clearing the in-memory ledger between games, capping `decisionCount` permanently at 1.

`GameStopped` carries a `GameResult` (WIN/LOSS/TIE/UNKNOWN) derived from the SC2 API `playerResult` at game end (#189). `GameOutcomeRecorder` maps WIN→ENDORSED (trust increases), LOSS→CHALLENGED (trust decreases), TIE→SOUND (neutral), UNKNOWN→skipped. Trust routing now receives directional signals from real games; Bayesian Beta scores evolve based on actual outcomes.

**Phase 1 migration (#193):** `StrategyTrustRouter` now implements `io.quarkmind.agent.TaskDefinition` with `getId() = "trust-routing"` and `execute(CaseContext ctx)` that writes the selected strategy ID to `agent.strategy.selected.id`. In Phase 1, `StrategyTrustObserver` (CDI event observer) still drives selection at game start and mid-game checkpoint; `execute()` writes the result to context for observability. In Phase 2, when `SequenceWorker` (engine#484) is available, the observer is deleted and the sequence step takes over entirely, making trust routing structural and event-log-visible.

Chapter entry: ✅ C5 (#198)
...
| Inter-plugin state within tick | None | None | CaseFile pre-populated by GameStateTranslator; DroolsStrategyTask writes decision key back; downstream plugins (tactics, economics) read it in the same tick |
...
### QuarkMind's own TaskDefinition — not promoted to engine API

**Date:** 2026-06-14
**Context:** Phase 1 migration introduced `io.quarkmind.agent.TaskDefinition` as the plugin contract for the production `casehub-engine` API. This interface could have been placed in `casehub-engine-api` as a platform-level Java-native worker definition helper.
**Decision:** `TaskDefinition` stays in QuarkMind. The engine's `Worker(name, caps, Function<CaseContext, Map>)` is already Java-native and sufficient for engine consumers. `TaskDefinition` lacks the cross-domain evidence required for a platform API. If AML, clinical, or devtown find it useful, promotion is the right path then.
**Consequences:** Plugin authors in other domains do not get `TaskDefinition` as a convenience layer. QuarkMind retains ownership. The interface is deleted if Phase 2 shows it was never needed beyond the migration period.

### CaseFileContext as Phase 1 migration shim

**Date:** 2026-06-14
**Context:** Phase 1 adds `execute(CaseContext)` to all plugins while keeping the poc `CaseEngine.createAndSolve()` path working. A bridge is needed to convert a poc `CaseFile` to `CaseContext` for the new method signatures.
**Decision:** `CaseFileContext` wraps a poc `CaseFile` as `CaseContext` by pre-extracting all 18 `QuarkMindCaseFile.ALL_KEYS` values at construction. It is explicitly labelled a Phase 1 shim (Javadoc says "removed in Phase 2"). Only bridge methods use it; the new `execute(CaseContext)` implementations receive a real `CaseContextImpl` in tests.
**Consequences:** The bridge is transparent — behavior is identical through both paths. `STRATEGY_SELECTED_ID = "agent.strategy.selected.id"` was added to `QuarkMindCaseFile` as a constant (not a raw literal) so Phase 2's `SequenceWorker` predicates can reference it compile-safely.
...
|---|---|---|
| `casehubio/ledger#114` — lightweight ledger mode | ~~High~~ Resolved | Shipped 2026-06-02. `DefaultOutcomeRecorder @DefaultBean` available; wire with `casehub.ledger.trust-score.routing-enabled=true`. C3, C4, C5 unblocked. |
| ~~`ocraft-s2client-bot`~~ transport removed (#185) | ~~Medium~~ Low | Vert.x 3.x / RxJava2 transport replaced by `QuarkusSC2Transport`. `ocraft-s2client-protocol` (data types only) remains — needs GraalVM 25 native verification. Tracked in `NATIVE.md`. |
