...
public enum MpqContent implements IMpqContent {
	 ...
	 * @param name optional name to use, if missing, the normalized enum name will be used
	...
	public static < T extends Enum< ? > & IMpqContent > String getDisplayName( final String name, final T mpqContent ) {
		return "<html>" + ( name == null ? constNameToNormal( mpqContent.name(), true ) : name ) + " (<code>" + mpqContent.getFileName() + "</code>)";
	}

	/**
	 * Converts a CONST_NAME style string to a normal display name (e.g. "CONST_NAME" -> "Const Name").
	 * Inlined from LUtils to avoid Scelight launcher dependency.
	 */
	private static String constNameToNormal( final String text, final boolean capitalizeWords ) {
