...
public class Player extends StructView implements IPlayer {
	...
	public Player( final Map< String, Object > struct, final int playerIdx ) {
		...
		// Strip these off
		name = stripOffMarkupFormatting( get( F_NAME ).toString().replace( "&lt;", "<" ).replace( "&gt;", ">" ) );
	...
	/** Strips off markup formatting tags from the specified string (e.g. {@code "<sp/>"}). */
	private static String stripOffMarkupFormatting( String s ) {
		int start = 0;
