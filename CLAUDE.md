...
- Tests: (continued) `OnnxLabelMappingTest`, `TemporalWindowAccumulatorTest`, `CascadeLatencyBenchmarkTest`
...
- Tests: (continued) `PatternClassificationRuleUnitTest`, `PatternClassificationCalibrationTest`, `CascadeValidationCalibrationTest`, `DominanceWeightRuleUnitTest`, `SC2CbrRetentionIT`, `SC2CbrRoutingIT`, `WorkbenchSocketIT`, `LlmPatternFallbackIT`, `CbrLearningCurveIT`
...
│     plugin/scouting/     DroolsScoutingTask, CascadingPatternClassifier, PatternClassificationRuleUnit
...
Eight benchmark tests run via `mvn test -pl quarkmind-sc2 -Pbenchmark`:
...
- `CascadeValidationCalibrationTest` — `@QuarkusTest` that runs 59 replays through three cascade modes (Drools-only, ONNX-only, cascade) at minutes 1-5. Reports per-matchup accuracy, tier hit rates, and comparison baselines. Asserts Drools-only rush accuracy ≥ 70% at minute 3.
- `CascadeLatencyBenchmarkTest` — plain JUnit. Measures feature extraction (p99 < 1ms), ONNX inference, and full cascade (p99 < 10ms) latency. Constructs ONNX models directly from test resources.
