...
- Tests: (continued) `PatternClassificationRuleUnitTest`, `PatternClassificationCalibrationTest`, `CascadeValidationCalibrationTest`, `DominanceWeightRuleUnitTest`, `SC2CbrRetentionIT`, `SC2CbrRoutingIT`, `WorkbenchSocketIT`, `LlmPatternFallbackIT`, `CbrLearningCurveIT`, `TemporalCbrRetentionIT`
...
- `TemporalMatchingCalibrationTest` — plain JUnit. Validates DTW self-retrieval (economy curve matches its own prefix over rush/turtle), DTW latency (< 10ms for 8x20 observations), and prediction extraction correctness.
