...
class GameArcSummariserTest {
    ...
    void producesNarrative_fromPhaseSequence() {
        var batch = List.of(
            new LevelEvent<>(new TacticalPosture("EARLY_MACRO", 0, "expanding"), 100, L3, "default"),
            new LevelEvent<>(new TacticalPosture("MID_SKIRMISH", 100, "battles"), 200, L3, "default"));
    ...
    void singlePhase_producesNarrative() {
        var batch = List.of(
            new LevelEvent<>(new TacticalPosture("DEFENSIVE_HOLD", 50, "under attack"), 100, L3, "default"));
        var arcs = summariser.summarise(batch).toCompletableFuture().join();
        assertThat(arcs).hasSize(1);
