...
public class CacheHandle implements ICacheHandle {
	...
	public CacheHandle( final XString value ) {
		type = new String( value.array, 0, 4, StandardCharsets.UTF_8 );
		regionCode = stripOffLeadingZeros( new String( value.array, 4, 4, StandardCharsets.UTF_8 ) );
		contentDigest = toHexString( value.array, 8, value.array.length - 8 );
	}
	...
	/** Converts a byte array slice to a lowercase hex string. */
	private static String toHexString( final byte[] data, final int offset, final int length ) {
