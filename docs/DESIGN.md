...
`SC2Data` in `domain/` provides shared constants for both `SimulatedGame` and `EmulatedGame`: damage-per-tick, attack range, supply cost, shield values, building health, mineral costs, and SC2 timing constants (`LOOPS_PER_TICK=22`, `GAME_LOOPS_PER_SECOND=22.4`). `trainTimeInLoops(UnitType)` returns empirically calibrated integer game-loop train durations (e.g. Probe=272 — SC2 stores these as integers, not as `seconds × 22.4`; values calibrated from 29 AI Arena replays via `SC2TrainTimeCalibrationTest`); `trainTimeInTicks` derives from it. `buildTimeInLoops(BuildingType)` returns empirically calibrated integer game-loop construction durations (e.g. Nexus=1600, Pylon=400 — previously stored as ticks, which was wrong by up to 32 ticks for Nexus; values calibrated from 30 AI Arena replays via `SC2BuildTimeCalibrationTest` using UnitInit→UnitDone tracker event diffs); `buildTimeInTicks` derives from it. `mineralIncomePerTick(int workerCount)` implements a per-base three-tier worker saturation model (50/25/5 min/min per worker, configurable via `MINERAL_PATCHES_PER_BASE` and `MINERAL_TIER_RATES_PER_TICK[]`); callers with multiple bases sum across each base's worker count. Worker income rate is race-invariant (Probe/SCV/Drone all mine at 5 minerals/trip). Centralised here to eliminate drift between engines. Also adds: `trainCount(UnitType)` (2 for ZERGLING, 1 for others), `muleIncomePerTick()` (Terran MULE flat income), `MULE_LIFETIME_LOOPS`, `QUEEN_ENERGY_REGEN_PER_LOOP`.
...
| `EmulatedGame` | Physics referee: holds `PlayerState friendly` + `PlayerState enemy`; both players share the same `applyIntent(Intent, PlayerState)` and `IntentQueue` drain path. Race-specific mechanics (initial state, larva regen, MULE calldown, Queen energy+inject, Overlord supply) are fully delegated to a `RaceModel` plugin — default is `ProtossRaceModel`; set via `setPlayerRaceModel(RaceModel)` before `reset()`. `applyIntent(TimedIntent)` preserves the absolute game loop for sub-tick completion precision; `startTraining` and `handleBuild` both use pure integer arithmetic `(loopOffset + SC2Data.trainTimeInLoops/buildTimeInLoops) / LOOPS_PER_TICK` — no float cast. `handleTrain()` uses a two-phase production check: `canProduce(PlayerStateView)` (structurally read-only, returns `ProductionDecision`, before resource deduction) then `onProductionCommitted()` (after deduction) — eliminates the ordering hazard where larva was consumed before the resource check. `handleMuleCalldown()` handles `MuleCalldownIntent` — validates OC present+complete, delegates to `playerRaceModel.onCalldown()`. `tick()` auto-computes worker counts per base via `countWorkersPerBase(RaceModel, buildings, units)` using squared-distance nearest-town-hall assignment; sums `SC2Data.mineralIncomePerTick` across each base; then calls `playerRaceModel.tickPassive()` for race-specific passive mechanics. `setMiningProbesPerBase(int... probesPerBase)` — one-shot override consumed by the next tick (defensively cloned). Harness injection paths: `injectReplayBuilding`, `injectReplayBuildingWithCost`, `markReplayBuildingComplete`, `setSupplyCapForHarness`, `setVespeneForHarness`. `drainBuildingQueues` propagates the previous unit's absolute completion loop to the next queued unit via `buildingCompletionAtLoop`, preserving sub-tick precision across queued training. |
| `RaceModel` | Plugin seam for race-specific `EmulatedGame` behaviour. Methods: `seedInitialState`, `tickPassive`, `canProduce` (takes `PlayerStateView`, returns `ProductionDecision`: PROCEED/BLOCKED — structurally read-only, #165), `onProductionCommitted`, `onUnitSpawned`, `onCalldown` (default no-op; handles direct ability-use spawns e.g. MULE; receives full `PlayerState`), `trainCount` (default 1; `ZergRaceModel` returns 2 for ZERGLING). `countWorkersPerBase(Race, …)` public overload uses cached per-race model instances. Forward-compatible with #74 (pluggable races). |
| `ProtossRaceModel` | `RaceModel` for Protoss: seeds 12 Probes + NEXUS; all production methods no-op. |
| `TerranRaceModel` | `RaceModel` for Terran: seeds 12 SCVs + CC; `canProduce` always returns PROCEED (no larva gate); MULE calldown via `MuleCalldownIntent` → `handleMuleCalldown` → `onCalldown` (spawns MULE at OC position, registers expiry); MULE expiry + flat income bonus in `tickPassive`. |
| `ZergRaceModel` | `RaceModel` for Zerg: seeds 12 Drones + HATCHERY + OVERLORD (supply=14); larva counter per hatchery with 245-loop regen; EGG unit spawned on morph commit (FIFO queue per hatchery); Queen energy regen + auto-inject in `tickPassive`; Overlord supply via `onUnitSpawned`. |
...
- **#152 Per-base probe distribution** — ✅ resolved: `tick()` auto-computes `miningProbesPerBase` from buildings/units; `countWorkersPerBase` (renamed from `countProbesPerBase`) extracted to `EmulatedGame` as canonical race-aware algorithm; one-shot override flag for replay harness
...
- ~~**#140 Terran replay files**~~ — **Done**: `AbilityMapping` populated with AI Arena build 75689 Terran constants from existing PvT replays (`ABIL_COMMAND_CENTER=155` → SCV, `ABIL_BARRACKS=159` → Marine/Marauder). `AbilityDiscoveryTest` extended with PvT replay coverage. `onSelection` rewritten to handle Scelight `removeMask` variants (ZeroIndices/Mask/None/OneIndices).
- ~~**#138 Terran/Zerg EmulatedGame**~~ — **Done**: `RaceModel` plugin seam introduced; `ProtossRaceModel`, `TerranRaceModel` (MULE), `ZergRaceModel` (larva/EGG/Queen inject) implemented. `countWorkersPerBase` replaces `countProbesPerBase`. `UnitType.EGG` added. `SC2Data` extended with all Terran/Zerg unit stats.
